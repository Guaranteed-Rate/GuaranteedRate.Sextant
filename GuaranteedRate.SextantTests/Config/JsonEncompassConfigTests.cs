...
namespace GuaranteedRate.Sextant.Config.Tests
{
    ...
    public class JsonEncompassConfigTests
    {
        ...
        {
            var json =
                "{\"foo\": \"bar\",\"boolkey\": \"false\",\"intkey\": \"4563\", \"Items\": [{\"FirstKey\": \"FirstVal\"}, {\"SecondKey\": \"SecondVal\"}] }";
            ...
            Assert.That(config.GetValue("foo").Equals("bar"),
                String.Format("Expected 3, got {0}", config.GetValue("foo")));
            Assert.That(config.GetValue("boolkey", true).Equals(false),
                String.Format("Expected 3, got {0}", config.GetValue("boolkey", true)));

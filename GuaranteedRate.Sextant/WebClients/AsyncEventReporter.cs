            ...
            Task.Run(() =>
            {
                while (!_eventQueue.IsCompleted)
                {
                    try
                    {
                        ...
                        successfulTake = _eventQueue.TryTake(out nextEvent);
                        if (successfulTake && nextEvent != null)
                        {
                            bool success = false;
                            int tries = 0;
                            ...
                            while (!success && tries < _retries)
                            {
                                ...
                                if (!success)
                                {
                                    Logger.Warn(Name, $"Write failed, try number: {tries}.");
                                }
                                tries++;
                            }
                            if (!success)
                    ...
                    {
                        Logger.Error(Name, $"Exception processing reporter queue: {ex}");
                        throw;
                    }
